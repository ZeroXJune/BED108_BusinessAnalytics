...
FULL_SQL = os.path.join(ROOT, "sql", "02_mysql_full_import.sql")
SCHEMA_MYSQL = os.path.join(ROOT, "sql", "01_schema_mysql.sql")
...
# YEAR_MONTH is a reserved word in MySQL/MariaDB, so the column has to be
# quoted in the INSERT column lists. Backticks are valid in SQLite as well.
RESERVED_COLUMNS = {"year_month"}
...
def insert_statements(tables):
    """Yield the INSERT statements for every table, in load order."""
    for name in LOAD_ORDER:
        header, rows = tables[name]
        yield f"-- {name}: {len(rows)} rows\n"
        cols = ", ".join(quote_column(c) for c in header)
        for i in range(0, len(rows), 100):
            chunk = rows[i:i + 100]
            values = ",\n    ".join(
                "(" + ", ".join(sql_literal(v) for v in row) + ")" for row in chunk
            )
            yield f"INSERT INTO {name} ({cols}) VALUES\n    {values};\n"
        yield "\n"
...
def write_inserts(tables):
    with open(INSERT_SQL, "w", encoding="utf-8") as fh:
        ...
        fh.write("--\n")
        fh.write("-- NOTE: this file contains no USE statement, so that it "
                 "also runs on SQLite.\n")
        fh.write("-- In MySQL you must select the database first:\n")
        fh.write("--     mysql -u root -p sales_trend < 03_insert_data.sql\n")
        fh.write("-- or run 02_mysql_full_import.sql instead, which is "
                 "self-contained.\n\n")
        for chunk in insert_statements(tables):
            fh.write(chunk)


def write_full_import(tables):
    """One self-contained file: create the database, the tables, and the data.

    This is the file to hand to phpMyAdmin's Import tab. It needs no
    prior setup and selects its own database.
    """
    with open(SCHEMA_MYSQL, encoding="utf-8") as fh:
        schema = fh.read()

    total = sum(len(rows) for _, rows in tables.values())
    with open(FULL_SQL, "w", encoding="utf-8") as fh:
        fh.write(
            "-- =====================================================================\n"
            "-- BED 106 Business Analytics - Mini Capstone: Sales Trend Analysis\n"
            "-- Checkpoint 1, Task 1.3: COMPLETE DATABASE IMPORT\n"
            "--\n"
            "-- ONE file that builds everything: creates the sales_trend database,\n"
            "-- creates all 8 tables with their keys and constraints, and loads all\n"
            f"-- {total:,} rows. Nothing needs to be set up first.\n"
            "--\n"
            "-- phpMyAdmin : Import tab -> Choose File -> this file -> Go.\n"
            "-- Workbench  : File -> Open SQL Script -> this file -> lightning bolt.\n"
            "-- Terminal   : mysql -u root -p < sql/02_mysql_full_import.sql\n"
            "--\n"
            "-- WARNING: the first statement DROPs any existing sales_trend\n"
            "-- database, so re-running this file discards previous changes.\n"
            "--\n"
            "-- Generated by scripts/clean_and_load.py; do not edit by hand.\n"
            "-- =====================================================================\n\n"
        )
        # Foreign key checks are disabled during load so the order of the
        # INSERT blocks can never cause a spurious failure, then re-enabled
        # and verified at the end.
        fh.write("SET FOREIGN_KEY_CHECKS = 0;\n")
        fh.write("SET NAMES utf8mb4;\n\n")
        fh.write(schema.rstrip() + "\n\n")
        fh.write("-- =====================================================================\n")
        fh.write("-- DATA\n")
        fh.write("-- =====================================================================\n\n")
        for chunk in insert_statements(tables):
        ...
        fh.write(
            "\nUNION ALL ".join(
                [f"SELECT '{name}' AS table_name, COUNT(*) AS row_count "
                 f"FROM {name}" for name in sorted(LOAD_ORDER)]
            ) + ";\n\n"
        )
        fh.write(
            "SELECT\n"
...
def main():
    ...
    print(f"Single-file MySQL import written to {FULL_SQL}")
